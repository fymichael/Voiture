CREATE OR REPLACE VIEW v_commission AS
    SELECT a.id_annonce, a.id_voiture, a."date" AS date_annonce,
           c.id_commission, c.valeur, c.date_changement,
           ABS(EXTRACT(EPOCH FROM (c.date_changement - a."date"))) AS diff_date
    FROM Annonce a
    CROSS JOIN LATERAL (
        SELECT c.id_commission, c.valeur, c.date_changement
        FROM Commission c 
        ORDER BY ABS(EXTRACT(EPOCH FROM (c.date_changement - a."date"))), c.date_changement
        LIMIT 1
    ) c;
